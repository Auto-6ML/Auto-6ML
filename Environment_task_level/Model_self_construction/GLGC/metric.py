...
def valid(model, device, dataset, view, data_size, class_num, mask, args):
    ...
    if args.choice in ["mask", "NI"]:
        ...
        for v in range(view):
            high_level_vectors[v] = high_level_vectors[v] * mask[:,v][:, np.newaxis]
        y_pred = kmeans.fit_predict(sum(high_level_vectors)/num[:, np.newaxis])
    elif args.choice == 'noise':
        y_pred = kmeans.fit_predict(sum(high_level_vectors) / view)
    else:
        print("搞错任务了")
