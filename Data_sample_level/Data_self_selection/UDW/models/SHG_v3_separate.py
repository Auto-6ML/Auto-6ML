...
class WeightedMLP(nn.Module):
    def __init__(self, args):
        ...
        self.w_list = nn.ModuleList([nn.Linear(cfg[-1] + args.deg_dim, cfg[-1], bias=True) for _ in range(view_num)]) #+ args.deg_dim
        ...
        self.mlp_edge_model = Sequential(
            Linear(cfg[-1] * 2 + args.deg_dim*2, 64), #+ args.deg_dim*2
            ReLU(),
            Linear(64, 1))
        # self.mlp_edge_model = Sequential(
        #     Linear(cfg[-1] * 2 + args.deg_dim * 2, 1))
        self.disc = Discriminator(self.args.cfg[-1] + self.args.deg_dim) #+ self.args.deg_dim
...
################################################################
    def forward(self, h_list, adj_list, h_a_list, knn):
        ...
        weight_n = 0
        for i in range(5000):
            weight_p += s[self.args.positive_edge[i][0], self.args.positive_edge[i][1]]
            weight_n += s[self.args.negative_edge[i][0], self.args.negative_edge[i][1]]
        print(weight_p/weight_n)
...
class SHG_v3(embedder):
    ...
    ###################################train model########################################
            ...
            for i in range(1):
                ...
                # loss_inv, loss_intra, std_loss = intra_inter_loss(h_p_list, fusion_emb, node_weight, g_score, self.args)
                ...
                # loss =  loss_inv + 10 * loss_intra + std_loss#+ loss_inv + self.args.fusionintra * loss_intra + self.args.con * sample__loss
...
##############################################train meta##############################################
            ...
            index0 = torch.sort(torch.Tensor(self.args.degree_list[0]))[1][0:2000].to(self.args.device)
            index1 = torch.sort(torch.Tensor(self.args.degree_list[0]))[1][2000:4000].to(self.args.device)
            ratio = torch.sum(node_weight[:,0,:][index1])/torch.sum(node_weight[:,0,:][index0])
            ratio_list.append(ratio.cpu().detach()+0.7)

            print(torch.sum(node_weight[:,0,:][index1])/torch.sum(node_weight[:,0,:][index0]))
            ...
            # recons_err, recons_nei = loss_matching_recons(recons, x_list, self.idx_p_list, self.args, epoch)
            loss_inv, loss_intra, std = intra_inter_loss(h_p_list, fusion_emb, node_weight, g_score, self.args)
            ...
            # loss_edge = loss_contrastive(fusion_emb, edge_weight, self.knn_edge, self.args)
            loss =  loss_dgi  + self.args.con * sample_loss #+  1 * loss_intra + 1 * std#+ self.args.con * loss_edge  #+ self.args.beta * (recons_err + recons_nei) + self.args.fusionintra * loss_intra
        ...
        print("aaaa")
        print(np.array(ratio_list))
        params = {
            'figure.figsize': '5.5, 6'
        }
        plt.rcParams.update(params)
        # plt.bar(range(len(data)), [47.9, 76.9, 83.3], 0.4, tick_label=labels, hatch="xxx")
        # # plt.bar(width, [67.3],0.3)
        # plt.ylabel('Micro-F1 (%)', fontsize=20)
        # # plt.xlabel('Size of the node degree',fontsize=20)
        # plt.xticks(fontproperties='Times New Roman', size=18)
        # plt.yticks(fontproperties='Times New Roman', size=18)
        plt.plot(np.array(ratio_list), 'b',
                 label='loss')  # epoch_losses 传入模型训练中的 loss[]列表,在训练过程中，先创建loss列表，将每一个epoch的loss 加进这个列表
        plt.ylabel('loss')
        plt.xlabel('epoch')
        plt.savefig("Deg_importance_ACM.pdf", dpi=300, bbox_inches='tight')
        plt.show()
        ...
        # plt.plot(np.array(ratio_list), 'b', label='loss')  # epoch_losses 传入模型训练中的 loss[]列表,在训练过程中，先创建loss列表，将每一个epoch的loss 加进这个列表
        # plt.ylabel('loss')
        # plt.xlabel('epoch')
        # plt.legend()  # 个性化图例（颜色、形状等）
        # # plt.savefig("Deg_affect.pdf", dpi=300, bbox_inches='tight')
        # plt.show()

        # index2 = torch.sort(torch.Tensor(self.args.degree_list[0]))[1][1008:3024].to(self.args.device)
        # number_node = 1008#adj1.shape[0]
        # random_split = np.random.permutation(number_node)
        # idx_train = random_split[:int(number_node * 0.2)]#.to(self.args.device)
        # idx_val = random_split[int(number_node * 0.2):int(number_node * 0.3)]#.to(self.args.device)
        # idx_test = random_split[int(number_node * 0.3):]#.to(self.args.device)
        #
        # self.idx_train = torch.LongTensor(idx_train).to(self.args.device)
        # self.idx_val = torch.LongTensor(idx_val).to(self.args.device)
        # self.idx_test = torch.LongTensor(idx_test).to(self.args.device)
        # self.labels = self.labels.squeeze()[index2].unsqueeze(dim=0)
...
def sample_decorrelation_loss(fusion_emb, edge_weight, knn, args):
    ...
    positive = torch.sum(value[value > value_positive]) / value[value>value_positive].shape[0]
    negative = torch.sum(value[value < value_negative]) / value[value > value_positive].shape[0]
    ...
    # repr_loss = 0
    # repr_loss += F.mse_loss(private_sample_0, private_sample_1)
    # repr_loss += -F.mse_loss(private_sample_2, private_sample_3)

    inter_c_positive = (private_sample_0) @ (private_sample_1).T
    inter_c_positive = F.normalize(inter_c_positive, p=2, dim=1)
    inter_c_positive = F.normalize(inter_c_positive, p=2, dim=1)
    loss_inv_positive = -torch.diagonal(inter_c_positive).mean()

    inter_c_negative = (private_sample_2) @ (private_sample_3).T
    inter_c_negative = F.normalize(inter_c_negative, p=2, dim=1)
    inter_c_negative = F.normalize(inter_c_negative, p=2, dim=1)
    loss_inv_negative = torch.diagonal(inter_c_negative).mean()
...
class trainer(nn.Module):
    ...
    def embed(self, x,  adj_list, adj_new):
        ...
        for i in range(self.args.view_num):
            ...
            # feature_encoding = self.Linear(h_list[i])
            # deg_enc_list.append(deg_encoding)
            h_a = torch.cat([h_a, deg_encoding], dim=1) #, deg_encoding
            if self.args.sparse:
                h_p = torch.spmm((adj_list[i] + self.args.adj_weight * adj_new), h_a)
            else:
                h_p = torch.mm((adj_list[i] + self.args.adj_weight * adj_new), h_a)
...
def intra_inter_loss(h_p_list, fusion_emb, node_weight, g_score, args):
    ...
    std_x = torch.sqrt(fusion_emb.var(dim=0) + 0.0001)
    # std_y = torch.sqrt(h_p_list[(i + 1) % args.view_num].var(dim=0) + 0.0001)
    std_loss = torch.mean(F.relu(1 - std_x)) #/ 2 + torch.mean(F.relu(1 - std_y)) / 2
