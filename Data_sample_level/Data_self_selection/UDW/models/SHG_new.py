...
class WeightedMLP(nn.Module):
    def __init__(self, args):
        ...
        self.w_list = nn.ModuleList([nn.Linear(cfg[-1] *4, cfg[-1], bias=True) for _ in range(view_num)])
...
################################################################
    def forward(self, h_list, adj_list, h_a_list):
        h_combine_list = []
        graph_score_list = []
        # edge_logits_list = []
        # meta_adj_list = []
...
##################edge weight####################################
        ...
        return h, score, g_score#, edge_logits_list
        #############################################

    def embed(self, h_list, adj_list, h_a_list):
        h_combine_list = []
        for i, h in enumerate(h_list):
            # deg_encoding = self.deg_enc_list[i](torch.LongTensor(self.args.degree_list[i])).to(self.args.device)
            # deg_enc_list.append(deg_encoding)
...
##############################################train meta##############################################
        ...
        hf = self.meta_model.embed(h_p_list, adj_list, h_a_list)
...
class trainer(nn.Module):
    def __init__(self, args):
        ...
        for i in range(self.args.view_num):
            ...
            self.deg_enc_list.append(nn.Embedding(int(max(self.args.degree_list[i]) + 1), self.args.cfg[-1], padding_idx=0))
        # for i in range(view_num):
