...
################################################################
    def forward(self, h_list, adj_list, h_a_list, knn):
        ...
        s = torch.zeros((feature_embedding.shape[0], feature_embedding.shape[0])).to(self.args.device)
        src, dst = knn
        ...
        edge_weight = torch.sigmoid(edge_logits).squeeze()
        ...
        for i in range(self.args.view_num):
            h_cat = torch.cat([h_list[i], h_new], dim=1)
            concat_list.append(h_cat)

        for i, h in enumerate(concat_list):
        ...
        h = torch.stack(concat_list, dim=1)
        h = g_score * score * h
        h = torch.sum(h, dim=1)
        ##############################
