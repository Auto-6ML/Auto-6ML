...
import os
from models import SHG_v3
import shutil

def get_args(model_name, dataset, custom_key="", yaml_path=None) -> argparse.Namespace:
    yaml_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), "args_v3.yaml")
    ...
    parser.add_argument('--meta_lr', type=float, default=0.0005, help='learning rate')
    ...
    parser.add_argument('--deg_dim', type=float, default=16, help='GCN self connection')
    parser.add_argument('--edge_rate', type=float, default=0.9, help='GCN self connection')
    parser.add_argument('--inv_view_weight', type=float, default=0, help='GCN self connection')
    parser.add_argument('--con', type=float, default=0.1, help='GCN self connection')
    parser.add_argument('--positive_rate', type=float, default=0.2, help='GCN self connection')
    parser.add_argument("--neighbor_num", default=300, help="Number of all sampled neighbor",
                        type=int)
    parser.add_argument("--adj_weight", default=0.05, help="Number of all sampled neighbor",
                        type=int)
    ...
    parser.add_argument('--fusionintra', type=float, default=0.05, help='weight on off-diagonal terms')
    ...
    parser.add_argument('--std_w', default=1, type=float, metavar='L', help='non-negative parameter')
    parser.add_argument("--tau", default=0.5, help="temperature in contrastive loss",
                        type=int)
...
def main():
    args = get_args(
        model_name="SHG",
        dataset="dblp", #acm, imdb, dblp, amazon
