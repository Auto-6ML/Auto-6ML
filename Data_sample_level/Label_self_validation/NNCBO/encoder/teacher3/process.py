...
def load_data(arg):
    '''加载数据'''
    if arg.dataset_name in ['Cora', 'CiteSeer', 'Pubmed']:
        path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.realpath(__file__)))) + '/utils', 'data')
        dataset = Planetoid(path, arg.dataset_name, pre_transform=T.NormalizeFeatures())
        data = dataset[0]
        data = data.to(arg.device)
        data.name = arg.dataset_name
        idx_train = torch.where(data.train_mask == True)[0].cpu().numpy()

    elif arg.dataset_name in ['dblp', 'Photo', 'Computers']:
        if arg.dataset_name == 'dblp':
            dataset = CitationFull('./data', 'dblp')
        elif arg.dataset_name in ['Photo', 'Computers']:
            dataset = Amazon('./data', arg.dataset_name)
        data = dataset[0]
        data = data.to(arg.device)
    ...
    # '''复制train_mask'''
    # train_idx = torch.zeros(len(data.train_mask), dtype=bool, device='cpu')
    # for i in range(len(data.train_mask)):
    #     if data.train_mask[i] == 1:
    #         train_idx[i] = 1
    #
    # '''复制保留正确标签，检测伪标签质量'''
    # y = torch.zeros(len(data.y)).to(arg.device)
    ...
    # noise_y = noisify_with_P(train_labels, nclass, ptb, 993, arg.noise)
    # noise_y = torch.tensor(noise_y)
    # data.y[idx_train] = noise_y.to(arg.device)
